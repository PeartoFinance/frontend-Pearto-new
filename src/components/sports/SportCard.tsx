...
import { MapPin, Clock, Play, Trophy } from 'lucide-react';
...
export function SportCard({ event, onClick }: SportCardProps) {
    ...
    return (
        <div
            className="bg-slate-800/50 border border-slate-700/50 rounded-xl overflow-hidden hover:border-emerald-500/50 transition-all cursor-pointer group"
            onClick={onClick}
        >
            ...
            <div className="p-4">
                <div className="flex items-center justify-between gap-4">
                    ...
                    <div className="flex-1 text-center">
                        <div className="w-12 h-12 mx-auto mb-2 bg-slate-700 rounded-full flex items-center justify-center text-lg font-bold text-white">
                            {event.teamHome?.slice(0, 3).toUpperCase() || '???'}
                        </div>
                        <p className="text-sm text-white font-medium truncate">{event.teamHome || 'TBD'}</p>
                    </div>
                    ...
                    <div className="flex-1 text-center">
                        ...
                        <p className="text-sm text-white font-medium truncate">{event.teamAway || 'TBD'}</p>
                    </div>
                </div>
