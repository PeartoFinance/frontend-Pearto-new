...
export default function AdvancedChartPage({ symbol, assetType = 'stock' }: AdvancedChartPageProps) {
    ...
    useEffect(() => {
        ...
        // Prepare data - deduplicate by time key to avoid duplicate timestamp errors
        const dataMap = new Map<string, typeof data[0]>();
        [...data]
            .filter(d => d.date && d.close != null)
            .sort((a, b) => a.date.localeCompare(b.date))
            .forEach(d => {
                const timeKey = d.date.split('T')[0]; // Use date part as key
                dataMap.set(timeKey, d); // Last value for each day wins
            });
        ...
        chart.subscribeCrosshairMove(param => {
            if (param.time && param.point) {
                const dataPoint = sortedData.find(d => d.date.split('T')[0] === param.time);
                if (dataPoint) {
        ...
        <div className="min-h-screen bg-slate-950 text-white flex flex-col">
            ...
            <div className="flex-1 flex overflow-hidden">
                ...
                <div className="flex-1 flex flex-col relative">
                    ...
                    {/* Volume chart */}
                    <div ref={volumeContainerRef} className="h-20 border-t border-slate-800" />
